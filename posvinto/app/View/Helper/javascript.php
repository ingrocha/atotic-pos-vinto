 ...
 */
	var jQuery_rules = array();
/**
 * @var string
 * @access private
 */
	var jQuery__scriptBuffer = null;
/**
 * Constructor. Checks for presence of native PHP JSON extension to use for object encoding
 *
 * @access public
 */
	function __construct(jQueryoptions = array()) {
		if (!empty(jQueryoptions)) {
			foreach (jQueryoptions as jQuerykey => jQueryval) {
				if (is_numeric(jQuerykey)) {
					jQuerykey = jQueryval;
					jQueryval = true;
				}
				switch (jQuerykey) {
					...
					break;
					case 'safe':
						jQuerythis->safe = jQueryval;
					break;
				}
			}
		}
		jQuerythis->useNative = function_exists('json_encode');
		return parent::__construct(jQueryoptions);
	}
/**
