...
<?php $pc=$insumo['Insumo']['preciocompra']; ?>
	...
	<?php echo $this->Form->text('salida', array('placeholder'=>'Introduzca la cantidad a sacar Ej: 2', 'required')); ?> Unidades
    
    <?php echo $this->Form->hidden('id_insumo', array('value'=>$id)); ?>
    <?php echo $this->Form->hidden('pc', array('value'=>$pc)); ?>
    <legend>Lugar:</legend>
    <p>
    <?php echo $this->Form->select('lugar',$lugares, array('placeholder'=>'Seleccione el Lugar', 'required'))  ?>
    </p>
    </fieldset>	
</div>
<?php else: ?>
