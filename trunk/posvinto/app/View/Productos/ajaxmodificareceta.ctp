<div class="widget widget-simple">
    <div class="widget-header">
        <h4>
            <i class="fontello-icon-user">
            </i>
            Modifica el insumo
            <small>
                <?php echo $receta['Insumo']['nombre'] ?>
            </small>
        </h4>
    </div>
    <div class="widget-content">
        <div class="widget-body">
            <?php echo $this->Form->create('Productos', array('id' => "accounForm", 'class' => 'form-horizontal'))
            ?>
            <div class="row-fluid">
                <div class="span12 form-dark">
                    <fieldset>
                        <ul class="form-list label-left list-bordered dotted">
                            <!-- // section form divider -->                          
                            <!-- // form item -->
                            <li class="control-group">
                                <label for="accountPrefix" class="control-label">
                                    Cantidad
                                </label>
                                <div class="controls">
                                    <?php
                                        echo $this->Form->hidden('Porcione.producto_id', array('value' => $receta['Producto']['id']));
                                    ?>
                                    <?php
                                        echo $this->Form->text('Porcione.cantidad', array('value'=>$receta['Porcione']['cantidad'], 'placeholder' => 'Cantidad numeral Ej.: 200'));
                                    ?>
                                </div>
                            </li>
                            <!-- // form item -->
                        </ul>
                    </fieldset>
                    <!-- // fieldset Input -->
                    <div class="form-actions">
                        <button type="submit" class="btn btn-blue">
                            Guardar
                        </button>
                    </div>
                </div>
            </div>
            </form>
        </div>
    </div>
</div>





<h1 class="tituloh1">Plato: <?php echo $receta['Producto']['nombre']; ?></h1>
<p>&nbsp;</p>
<h2 class="tituloh3">Insumo: <?php echo $receta['Insumo']['nombre']; ?></h2>
<?php //debug($receta); ?>
<br />
<p>&nbsp;</p>
<?php echo $this->Form->create('Receta'); ?>
<?php echo $this->Form->hidden('id', array('size'=>5, 'value'=>$receta['Porcione']['id'])); ?>
<b>Cantidad: </b><?php echo $this->Form->text('cantidad', array('size'=>5, 'value'=>$receta['Porcione']['cantidad'])); ?>
<p>&nbsp;</p>
<?php echo $this->Form->end('Modificar'); ?>
